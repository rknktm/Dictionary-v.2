...
def delete(key):
  ...
  with open("words.txt","w",encoding = "utf-8") as file:
      for k,v in dicto.items():
        if k == key:
          if "," in dicto[key]:         
              responce = input(f"Which antonym of the word would you like to delete? {dicto[key]}:")
              responce = responce.capitalize()
              ...
              sleep(2)
              return f"{key} ==> \N{sauropod} ==> {responce} word-pair has been succesfully deleted from the dictionary."
          else:
            print("Please wait while word-pair is deleted from the dictionary. ")
            sleep(2)
            return f"{key} ==> \N{sauropod} ==> {dicto[key]} word-pair has been succesfully deleted from the dictionary."
        else:
          word = (k+" "+v+"\n")
          file.write(word)
# 3-updating word 
def update(key):
  ...
  if responce == "UPS :(( word coundn't be found.":
    ...
    while True:
      ant = input("Would you like save these word to the dictionary? \N{sauropod} ==> y/n:")
      ant = ant.capitalize()
      if ant == "Y" :
          newv= input("Please write antonym of the word: ")
      ...
      if "," in responce: 
          word = input(f"Which antonym of the word would you like to update? {responce}:")
